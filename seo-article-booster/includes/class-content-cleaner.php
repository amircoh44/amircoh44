...
/**
 * Class SAB_Content_Cleaner
 */
class SAB_Content_Cleaner {

	/** Post meta holding the pre-clean backup of post_content. */
	const META_BACKUP      = '_sab_cleaner_backup';
	const META_BACKUP_TIME = '_sab_cleaner_backup_time';

	/** Option holding the cleaning log. */
	const LOG_OPTION = 'sab_cleaner_log';
	...
	public static function cleanups() {
		return array(
			'clean_code_fences'     => __( 'Markdown code fences (``` … ```)', 'seo-article-booster' ),
			'clean_zero_width'      => __( 'Zero-width & invisible characters', 'seo-article-booster' ),
			'clean_empty_paragraphs' => __( 'Empty paragraphs', 'seo-article-booster' ),
			'clean_word_cruft'      => __( 'Word/Office cruft (<o:p>, <font>, conditional comments)', 'seo-article-booster' ),
			'clean_script_style'    => __( 'Stray <script> and <style> blocks', 'seo-article-booster' ),
			'clean_multiple_br'     => __( 'Runs of three or more <br>', 'seo-article-booster' ),
			'clean_empty_tags'      => __( 'Empty inline tags (<span></span>, …)', 'seo-article-booster' ),
			'clean_html_comments'   => __( 'HTML comments (Gutenberg blocks preserved)', 'seo-article-booster' ),
			'clean_inline_styles'   => __( 'Inline style="" attributes (CSS)', 'seo-article-booster' ),
			'clean_classes'         => __( 'class="" attributes (CSS hooks — may affect layout/blocks)', 'seo-article-booster' ),
		);
	}
	...
	protected function on( $key, $force ) {
		...
		return (bool) SAB_Settings::get( $key );
	}
	...
	public function revert_post( $post_id ) {
		if ( ! metadata_exists( 'post', $post_id, self::META_BACKUP ) ) {
			return new WP_Error( 'sab_no_backup', __( 'No backup is available for this post.', 'seo-article-booster' ) );
		}
	...
	protected function query( $paged, $per_page ) {
		return new WP_Query(
			array(
				'post_type'           => (array) SAB_Settings::get( 'link_post_types', array( 'post', 'page' ) ),
	...
	public function on_insert_post_data( $data, $postarr ) {
		...
		if ( ! in_array( $data['post_type'], (array) SAB_Settings::get( 'link_post_types', array( 'post', 'page' ) ), true ) ) {
			return $data;
		}
