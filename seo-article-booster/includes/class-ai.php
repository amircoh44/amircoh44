...
class SAB_AI {
	...
	public static function is_configured() {
		return '' !== trim( (string) SAB_Settings::get( 'ai_endpoint' ) )
			&& '' !== trim( (string) SAB_Settings::get( 'ai_key' ) );
	}
	...
	public static function generate( $system, $user, $max_tokens = 220 ) {
		if ( ! self::is_configured() ) {
			return new WP_Error( 'sab_ai_unconfigured', __( 'Add your AI endpoint and API key in Settings → AI first.', 'seo-article-booster' ) );
		}

		$endpoint = trim( (string) SAB_Settings::get( 'ai_endpoint' ) );
		$model    = trim( (string) SAB_Settings::get( 'ai_model' ) );
		 ...
		 */
		$body = apply_filters( 'sab_ai_request_body', $body );
		...
		$response = wp_remote_post(
			...
			array(
				'timeout' => 30,
				'headers' => array(
					'Authorization' => 'Bearer ' . SAB_Settings::get( 'ai_key' ),
		...
		if ( $code < 200 || $code >= 300 ) {
			$message = isset( $data['error']['message'] ) ? $data['error']['message'] : sprintf( /* translators: %d: HTTP status. */ __( 'AI request failed (HTTP %d).', 'seo-article-booster' ), $code );
			return new WP_Error( 'sab_ai_http', $message );
		}
		...
		return new WP_Error( 'sab_ai_parse', __( 'Unexpected response from the AI provider.', 'seo-article-booster' ) );
	}
