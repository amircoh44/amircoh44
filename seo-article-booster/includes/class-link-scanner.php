...
/**
 * Class SAB_Link_Scanner
 */
class SAB_Link_Scanner {

	const META_INTERNAL = '_sab_links_internal';
	const META_EXTERNAL = '_sab_links_external';
	...
	public function update_link( $post_id, $index, $url, $html ) {
		...
		if ( ! $post ) {
			return new WP_Error( 'sab_no_post', __( 'Post not found.', 'seo-article-booster' ) );
		}
		...
		if ( mb_strlen( trim( wp_strip_all_tags( $inner ) ) ) > self::MAX_ANCHOR_CHARS ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'The link text must be %d characters or fewer.', 'seo-article-booster' ),
					self::MAX_ANCHOR_CHARS
				)
			);
		}
		if ( '' === trim( wp_strip_all_tags( $inner ) ) ) {
			return new WP_Error( 'sab_empty', __( 'The link text cannot be empty.', 'seo-article-booster' ) );
		}
		...
		if ( ! $found || null === $new_content ) {
			return new WP_Error( 'sab_not_found', __( 'That link could not be located (the content may have changed). Please reload.', 'seo-article-booster' ) );
		}
	...
	public function on_save_post( $post_id, $post ) {
		...
		if ( ! in_array( $post->post_type, (array) SAB_Settings::get( 'link_post_types', array( 'post', 'page' ) ), true ) ) {
			return;
		}
	...
	public function register_columns() {
		foreach ( (array) SAB_Settings::get( 'link_post_types', array( 'post', 'page' ) ) as $type ) {
	...
	public function add_column( $columns ) {
		$columns['sab_links'] = __( 'Links (int / ext)', 'seo-article-booster' );
		return $columns;
	}
	...
	public function render_column( $column, $post_id ) {
		...
		$internal_badge = sprintf(
			'<span class="sab-badge %s" title="%s">%d</span>',
			$counts['internal'] > 0 ? 'sab-badge--ok' : 'sab-badge--warn',
			esc_attr__( 'Internal links', 'seo-article-booster' ),
			(int) $counts['internal']
		);
		$external_badge = sprintf(
			'<span class="sab-badge sab-badge--neutral" title="%s">%d</span>',
			esc_attr__( 'External links', 'seo-article-booster' ),
			(int) $counts['external']
		);

		printf(
			'%s %s <a href="%s" class="sab-muted">%s</a>',
			...
			esc_url( $view ),
			esc_html__( 'view', 'seo-article-booster' )
		);
	}
}
