...
class SAB_Syndication {
	...
	public function on_transition( $new_status, $old_status, $post ) {
		...
		if ( ! SAB_Settings::get( 'syndicate_enabled' ) ) {
			return;
		}
		if ( ! SAB_Edition::can( 'syndication' ) ) {
			return; // Expert feature.
		}
		if ( ! $post instanceof WP_Post ) {
			return;
		}
		if ( ! in_array( $post->post_type, (array) SAB_Settings::get( 'syndicate_post_types', array( 'post' ) ), true ) ) {
			return;
		}
		$this->dispatch( $post );
	}
	...
	public function webhooks() {
		$raw  = (string) SAB_Settings::get( 'syndicate_webhooks', '' );
		$urls = array();
		foreach ( preg_split( '/\r\n|\r|\n/', $raw ) as $line ) {
			$line = trim( $line );
			if ( '' !== $line ) {
	...
	public function build_payload( $post ) {
		 ...
		 * @param WP_Post $post    Post.
		 */
		return apply_filters( 'sab_syndication_payload', $payload, $post );
	}
	...
	public function dispatch( $post ) {
		 ...
		 */
		do_action( 'sab_syndicated', $post_obj ? $post_obj->ID : 0, $urls );
