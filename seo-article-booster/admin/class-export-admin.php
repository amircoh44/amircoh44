...
/**
 * Class SAB_Export_Admin
 */
class SAB_Export_Admin {

	const CAP   = 'export';
	const PAGE  = 'sab-export';
	const NONCE = 'sab_export';
	 ...
	 * Constructor.
	 *
	 * @param SAB_Exporter $exporter Exporter.
	 */
	public function __construct( SAB_Exporter $exporter ) {
	...
	public function init() {
		...
		add_action( 'admin_post_sab_export_json', array( $this, 'download_json' ) );
		add_action( 'admin_post_sab_export_zip_download', array( $this, 'download_zip' ) );
		add_action( 'wp_ajax_sab_export_zip_start', array( $this, 'ajax_zip_start' ) );
		add_action( 'wp_ajax_sab_export_zip_batch', array( $this, 'ajax_zip_batch' ) );
	}
	...
	public function register_menu() {
		$this->screen = add_submenu_page(
			'sab-dashboard',
			__( 'Export / Migrate', 'seo-article-booster' ),
			__( 'Export / Migrate', 'seo-article-booster' ),
			self::CAP,
			self::PAGE,
			array( $this, 'render' )
	...
	public function enqueue( $hook ) {
		...
		wp_enqueue_script( 'sab-export', SAB_PLUGIN_URL . 'admin/js/export.js', array( 'jquery' ), SAB_VERSION, true );
		wp_localize_script(
			'sab-export',
			'SAB_EXPORT',
			array(
				...
				'i18n'        => array(
					'preparing' => __( 'Preparing…', 'seo-article-booster' ),
					'adding'    => __( 'Adding media…', 'seo-article-booster' ),
					'ready'     => __( 'ZIP ready — downloading…', 'seo-article-booster' ),
					'error'     => __( 'Something went wrong.', 'seo-article-booster' ),
					'needAuth'  => __( 'Please tick the authorisation box first.', 'seo-article-booster' ),
				),
			)
		);
	}
	...
	public function render() {
		if ( ! current_user_can( self::CAP ) ) {
			wp_die( esc_html__( 'You do not have permission to export this site.', 'seo-article-booster' ) );
		}
	...
	protected function guard_request( $src, $nonce_field, $nonce_value ) {
		...
		if ( $this->has_pii( $opts ) && empty( $src['authorize'] ) ) {
			wp_die( esc_html__( 'This export includes personal data. You must confirm you are authorised to export it.', 'seo-article-booster' ), 403 );
		}
		return $opts;
	}
	...
	public function download_json() {
		...
		$opts = $this->guard_request( $src, 'sab_export_nonce', self::NONCE );
	...
	protected function ajax_guard() {
		...
		if ( $this->has_pii( $opts ) && empty( $_POST['authorize'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Confirm you are authorised to export personal data first.', 'seo-article-booster' ) ), 403 );
		}
		return $opts;
	}
	...
	public function ajax_zip_start() {
		...
		$path  = trailingslashit( get_temp_dir() ) . 'sab-export-' . $token . '.zip';

		$zip = new ZipArchive();
		if ( true !== $zip->open( $path, ZipArchive::CREATE | ZipArchive::OVERWRITE ) ) {
			wp_send_json_error( array( 'message' => __( 'Could not create the ZIP file.', 'seo-article-booster' ) ) );
		}
		$json = wp_json_encode( $this->exporter->build_manifest( $opts ), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
	...
	public function ajax_zip_batch() {
		...
		$state = $token ? get_transient( 'sab_export_' . $token ) : false;
		if ( ! is_array( $state ) ) {
			wp_send_json_error( array( 'message' => __( 'Export session expired. Please start again.', 'seo-article-booster' ) ) );
		}

		$per_batch = 20;
		$zip       = new ZipArchive();
		if ( true !== $zip->open( $state['path'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Could not open the ZIP file.', 'seo-article-booster' ) ) );
		}
		...
		$state['index'] = $end;
		set_transient( 'sab_export_' . $token, $state, HOUR_IN_SECONDS );
	...
	public function download_zip() {
		...
		if ( ! wp_verify_nonce( $nonce, self::NONCE ) ) {
			wp_die( esc_html__( 'Security check failed.', 'seo-article-booster' ), 403 );
		}
		...
		if ( ! is_array( $state ) || empty( $state['path'] ) || ! file_exists( $state['path'] ) ) {
			wp_die( esc_html__( 'Export not found or expired.', 'seo-article-booster' ), 404 );
		}
		...
		wp_delete_file( $path );
		delete_transient( 'sab_export_' . $token );
		exit;
	}
	...
	protected function filename( $ext ) {
		...
		$slug = $slug ? $slug : 'site';
		return 'sab-export-' . $slug . '-' . gmdate( 'Ymd-His' ) . '.' . $ext;
	}
}
