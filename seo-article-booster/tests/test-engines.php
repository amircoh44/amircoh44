...
$force   = array();
foreach ( array_keys( SAB_Content_Cleaner::cleanups() ) as $k ) { $force[ $k ] = 1; }
...
check( 'backup meta saved', metadata_exists( 'post', $pid2, '_sab_cleaner_backup' ) );
$log = $cleaner->get_log();
...
check( 'backup removed after revert', ! metadata_exists( 'post', $pid2, '_sab_cleaner_backup' ) );

// Link replacer (display-time injection engine).
$rep   = new SAB_Link_Replacer();
$entry = array(
	'post_id' => 123,
...
// Settings sanity.
check( 'default min_images = 3', 3 === (int) SAB_Settings::get( 'min_images' ) );
check( 'clean_inline_styles default ON', 1 === (int) SAB_Settings::get( 'clean_inline_styles' ) );
check( 'clean_classes default OFF', 0 === (int) SAB_Settings::get( 'clean_classes' ) );
