...
SAB_Business_Profile::flush_cache();

// Profile helpers.
check( 'schema_type resolves LocalBusiness subtype', 'Plumber' === SAB_Business_Profile::schema_type() );
check( 'same_as collects social URLs', count( SAB_Business_Profile::same_as() ) === 2 );
check( 'opening hours parse to 2 specs', count( SAB_Business_Profile::opening_hours_spec() ) === 2 );
check( 'profile is complete', SAB_Business_Profile::is_complete() );

$gen = new SAB_Schema_Generator();
...
$urls = SAB_Settings::get_sitemap_urls();
