...
class SPR_Content_Distributor {
	 ...
	 */
	const WRAP_CLASS = 'spr-injection';
	...
	public function enqueue_public_styles() {
		...
		wp_enqueue_style( 'spr-public', SPR_PLUGIN_URL . 'public/css/public.css', array(), SPR_VERSION );
	}
	...
	protected function should_run() {
		if ( ! SPR_Settings::get( 'enable_distribution', 1 ) ) {
			return false;
		}
		if ( ! SPR_Edition::can( 'distribution' ) ) {
		...
		return (bool) apply_filters( 'spr_should_distribute', true );
	}
	...
	protected function render_image( $rule ) {
		...
		$attr = array( 'class' => 'spr-injection-img' );
	...
	protected function build_payload_node( $dom, $rule ) {
		...
		$wrapper->setAttribute( 'data-spr-rule', (string) $rule['id'] );
