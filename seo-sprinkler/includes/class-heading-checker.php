...
/**
 * Class SPR_Heading_Checker
 */
class SPR_Heading_Checker {

	/** Post meta flag: this post's H1 warning is dismissed. */
	const META_IGNORE = '_spr_ignore_h1';
	...
	public function init() {
		...
		add_action( 'wp_ajax_spr_ignore_h1', array( $this, 'ajax_ignore' ) );
	}
	...
	protected function applies_to( $post_type ) {
		return in_array( $post_type, (array) SPR_Settings::get( 'link_post_types', array( 'post', 'page' ) ), true );
	}
	...
	public function maybe_notice() {
		...
		?>
		<div class="notice notice-warning spr-h1-notice" data-post="<?php echo esc_attr( $post->ID ); ?>">
			<p>
				<strong><?php esc_html_e( 'SEO Sprinkler:', 'seo-sprinkler' ); ?></strong>
				<?php
				printf(
					/* translators: %d: number of H1 tags found in content. */
					esc_html( _n( 'This content contains %d <h1> heading. Themes usually output the post title as the page’s only H1, so this can create a duplicate-H1 SEO issue.', 'This content contains %d <h1> headings. Themes usually output the post title as the page’s only H1, so these can create duplicate-H1 SEO issues.', $count, 'seo-sprinkler' ) ),
					(int) $count
				);
				?>
				<?php esc_html_e( 'If this is intentional (for example, you hide the theme H1 and use your own), you can ignore it.', 'seo-sprinkler' ); ?>
			</p>
			<p>
				<button type="button" class="button button-small spr-h1-ignore"><?php esc_html_e( 'Ignore for this post', 'seo-sprinkler' ); ?></button>
			</p>
		</div>
		<?php
	}
	...
	public function publish_box( $post ) {
		...
		?>
		<div class="misc-pub-section spr-pub-h1" data-post="<?php echo esc_attr( $post->ID ); ?>">
			<span class="dashicons <?php echo $ignored ? 'dashicons-hidden' : 'dashicons-warning'; ?>"></span>
			<?php
			printf(
				/* translators: %d: number of content H1 tags. */
				esc_html__( 'Content H1: %d', 'seo-sprinkler' ),
				(int) $count
			);
			?>
			<?php if ( $ignored ) : ?>
				— <a href="#" class="spr-h1-restore"><?php esc_html_e( 'show warning', 'seo-sprinkler' ); ?></a>
			<?php endif; ?>
		</div>
		<?php
	}
	...
	public function enqueue( $hook ) {
		...
		wp_enqueue_script( 'spr-h1', SPR_PLUGIN_URL . 'admin/js/h1-notice.js', array( 'jquery' ), SPR_VERSION, true );
		wp_localize_script(
			'spr-h1',
			'SPR_H1',
			array(
				...
				'ignored' => __( 'Warning ignored for this post.', 'seo-sprinkler' ),
			)
		);
	}
