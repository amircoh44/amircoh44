...
class SPR_Link_Index {
	 ...
	 */
	public function __construct( SPR_Sitemap_Parser $sitemap ) {
	...
	public function get_index( $force = false ) {
		...
		if ( ! $force ) {
			$cached = get_transient( SPR_TRANSIENT_INDEX );
			if ( is_array( $cached ) ) {
		...
		$raw = $this->build();
		set_transient( SPR_TRANSIENT_INDEX, $raw, DAY_IN_SECONDS );
	...
	protected function build() {
		$post_types = (array) SPR_Settings::get( 'link_post_types', array( 'post', 'page' ) );
		$min_len    = (int) SPR_Settings::get( 'min_keyword_length', 4 );
		$restrict   = (bool) SPR_Settings::get( 'restrict_to_sitemap', 1 );

		// Map of normalised sitemap URLs for fast lookup.
		$sitemap_lookup = array();
		if ( $restrict ) {
			foreach ( $this->sitemap->get_all_urls() as $url ) {
				$sitemap_lookup[ SPR_Sitemap_Parser::normalize_url( $url ) ] = true;
			}
		...
		foreach ( $query->posts as $post_id ) {
			...
			if ( $restrict && ! isset( $sitemap_lookup[ SPR_Sitemap_Parser::normalize_url( $url ) ] ) ) {
		 ...
		 */
		return apply_filters( 'spr_link_index', $entries );
	}
	...
	protected function phrases_for_post( $post_id ) {
		...
		if ( SPR_Settings::get( 'use_title_as_keyword' ) ) {
		...
		if ( SPR_Settings::get( 'use_yoast_focus_kw' ) ) {
		 ...
		 * @param int      $post_id Post ID.
		 */
		return array_unique( apply_filters( 'spr_post_phrases', $phrases, $post_id ) );
	}
	...
	protected function compile( $raw ) {
		$case_flag = SPR_Settings::get( 'case_sensitive' ) ? '' : 'i';
