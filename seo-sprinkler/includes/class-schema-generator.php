...
class SPR_Schema_Generator {
	...
	public function output() {
		if ( ! SPR_Settings::get( 'enable_schema_output' ) ) {
			return;
		}
		if ( ! SPR_Edition::can( 'schema_output' ) ) {
		...
		echo "\n<script type=\"application/ld+json\" class=\"spr-schema-graph\">"
	...
	public function build_graph() {
		 ...
		 */
		return apply_filters( 'spr_schema_graph', $graph );
	}
	...
	protected function organization_node() {
		if ( ! SPR_Business_Profile::is_complete() ) {
		...
		$type = SPR_Business_Profile::schema_type();

		$node = array(
			'@type' => $type,
			'@id'   => $this->org_id(),
			'name'  => SPR_Business_Profile::get( 'name' ),
			'url'   => SPR_Business_Profile::get( 'url' ) ? SPR_Business_Profile::get( 'url' ) : home_url( '/' ),
		);
		...
		foreach ( array(
			...
			$val = trim( (string) SPR_Business_Profile::get( $field ) );
			if ( '' !== $val ) {
				$node[ $prop ] = $val;
			}
		}
		...
		// Logo + image.
		if ( SPR_Business_Profile::get( 'logo_id' ) ) {
		...
		$lat = SPR_Business_Profile::get( 'latitude' );
		$lng = SPR_Business_Profile::get( 'longitude' );
		if ( '' !== $lat && '' !== $lng ) {
			$node['geo'] = array(
				'@type'     => 'GeoCoordinates',
				'latitude'  => (float) $lat,
		...
		// Opening hours.
		$hours = SPR_Business_Profile::opening_hours_spec();
		if ( ! empty( $hours ) ) {
			$node['openingHoursSpecification'] = $hours;
		}

		// Area served.
		$area = trim( (string) SPR_Business_Profile::get( 'area_served' ) );
		if ( '' !== $area ) {
			$node['areaServed'] = array_map( 'trim', explode( ',', $area ) );
		}

		// Founder.
		$founder = trim( (string) SPR_Business_Profile::get( 'founder' ) );
		if ( '' !== $founder ) {
			$node['founder'] = array(
				'@type' => 'Person',
				'name'  => $founder,
		...
		// Social profiles.
		$same_as = SPR_Business_Profile::same_as();
		if ( ! empty( $same_as ) ) {
			$node['sameAs'] = $same_as;
		}

		// Contact point.
		$tel = trim( (string) SPR_Business_Profile::get( 'telephone' ) );
		if ( '' !== $tel ) {
			$node['contactPoint'] = array(
				'@type'       => 'ContactPoint',
				'telephone'   => $tel,
				'contactType' => SPR_Business_Profile::get( 'contact_type' ) ? SPR_Business_Profile::get( 'contact_type' ) : 'customer service',
			);
		}
	...
	protected function logo_node() {
		$id = SPR_Business_Profile::get( 'logo_id' );
		if ( ! $id ) {
			return null;
		}
	...
	protected function primary_entity_node( $post_id, $webpage_id, $url ) {
		...
		$service   = (string) SPR_Settings::get( 'service_post_type', 'service' );
		...
		$article_types = (array) apply_filters( 'spr_article_post_types', array( 'post' ) );
	...
	protected function breadcrumb_node( $url ) {
		$items = array();
		$items[] = array( 'name' => __( 'Home', 'seo-sprinkler' ), 'url' => home_url( '/' ) );
