...
class SPR_Image_Scanner {
	...
	public function count_for_post( $post ) {
		...
		if ( SPR_Settings::get( 'count_featured_image' ) && has_post_thumbnail( $post ) ) {
			$count++;
		}
	...
	public function get_minimum() {
		return (int) SPR_Settings::get( 'min_images', 0 );
	}
	...
	public function on_save_post( $post_id, $post, $update = false ) {
		...
		if ( ! in_array( $post->post_type, (array) SPR_Settings::get( 'audit_post_types', array() ), true ) ) {
			return;
		}
	...
	public function get_cached_count( $post_id ) {
		$stored = get_post_meta( $post_id, SPR_META_IMAGE_COUNT, true );
		if ( '' === $stored ) {
	...
	public function scan_batch( $paged = 1, $per_page = 50 ) {
		$post_types = (array) SPR_Settings::get( 'audit_post_types', array( 'post' ) );
	...
	public function register_admin_columns() {
		foreach ( (array) SPR_Settings::get( 'audit_post_types', array() ) as $type ) {
	...
	public function add_column( $columns ) {
		$columns['spr_images'] = __( 'Images', 'seo-sprinkler' );
		return $columns;
	}
	...
	public function render_column( $column, $post_id ) {
		...
		if ( $count < $min ) {
			printf(
				'<span class="spr-badge spr-badge--warn" title="%s"><span class="dashicons dashicons-warning"></span> %d / %d</span>',
				esc_attr__( 'Below the minimum number of images', 'seo-sprinkler' ),
				(int) $count,
				(int) $min
			);
		} else {
			printf(
				'<span class="spr-badge spr-badge--ok"><span class="dashicons dashicons-yes"></span> %d</span>',
				(int) $count
			);
		}
	}
	...
	public function make_column_sortable( $columns ) {
		$columns['spr_images'] = 'spr_images';
		return $columns;
	}

	/**
	 * Translate the "spr_images" orderby into a meta-value sort.
	 *
	...
	public function handle_column_sorting( $query ) {
		...
		if ( 'spr_images' !== $query->get( 'orderby' ) ) {
			return;
		}
		$query->set( 'meta_key', SPR_META_IMAGE_COUNT );
	...
	public function maybe_show_editor_notice() {
		...
		if ( ! in_array( $screen->post_type, (array) SPR_Settings::get( 'audit_post_types', array() ), true ) ) {
			return;
		}
		...
		printf(
			...
			esc_html__( 'SEO Sprinkler:', 'seo-sprinkler' ),
			esc_html(
				sprintf(
					/* translators: 1: current image count, 2: required minimum. */
					__( 'This content currently has %1$d image(s) but the configured minimum is %2$d. Consider adding more images.', 'seo-sprinkler' ),
					$count,
					$min
				)
			)
	...
	public function render_publish_box_status( $post ) {
		...
		printf(
			'<div class="misc-pub-section spr-pub-images"><span class="dashicons %s"></span> %s <strong>%d</strong> / %d</div>',
			esc_attr( $ok ? 'dashicons-yes-alt' : 'dashicons-warning' ),
			esc_html__( 'Images:', 'seo-sprinkler' ),
			(int) $count,
			(int) $min
		);
	}
