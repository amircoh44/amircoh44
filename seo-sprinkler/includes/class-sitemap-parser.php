...
class SPR_Sitemap_Parser {
	...
	public function get_all_urls( $force = false ) {
		if ( ! $force ) {
			$cached = get_transient( SPR_TRANSIENT_SITEMAP );
			if ( is_array( $cached ) ) {
				return $cached;
			}
		}
		...
		set_transient( SPR_TRANSIENT_SITEMAP, $urls, empty( $urls ) ? MINUTE_IN_SECONDS * 15 : self::CACHE_TTL );
	...
	public function fetch_all_urls() {
		...
		foreach ( SPR_Settings::get_sitemap_urls() as $sitemap_url ) {
	...
	protected function fetch_one( $url ) {
		...
		if ( ! $root ) {
			$this->last_error = __( 'A sitemap could not be parsed as valid XML.', 'seo-sprinkler' );
			return array();
		}
		...
		if ( 'sitemapindex' === $root->getName() ) {
			...
			$this->last_error = __( 'Unrecognised sitemap format (expected sitemapindex or urlset).', 'seo-sprinkler' );
		}
	...
	protected function fetch( $url ) {
		$response = wp_remote_get(
			...
			array(
				...
				'sslverify'  => apply_filters( 'spr_sitemap_sslverify', true ),
			)
		);
		...
		if ( is_wp_error( $response ) ) {
			$this->last_error = sprintf(
				...
				__( 'Could not fetch the sitemap: %s', 'seo-sprinkler' ),
		...
		if ( $code < 200 || $code >= 300 ) {
			$this->last_error = sprintf(
				...
				__( 'The sitemap request returned HTTP status %d.', 'seo-sprinkler' ),
				$code
			);
			return false;
		}
		...
		if ( '' === trim( (string) $body ) ) {
			$this->last_error = __( 'The sitemap response was empty.', 'seo-sprinkler' );
			return false;
		}
