...
class SPR_Schema_Scanner {
	 ...
	 */
	const META_TYPES   = '_spr_schema_types';
	const META_CHECKED = '_spr_schema_checked';

	/**
	 * Optional sitemap parser (for the sitemap-coverage audit).
	 *
	 * @var SPR_Sitemap_Parser|null
	 */
	 ...
	 * Constructor.
	 *
	 * @param SPR_Sitemap_Parser|null $sitemap Sitemap parser.
	 */
	...
	public function detect_for_post( $post ) {
		...
		if ( ! $post ) {
			return array(
				'checked' => false,
				'types'   => array(),
				'error'   => __( 'Post not found.', 'seo-sprinkler' ),
			);
		}
	...
	public function detect_for_url( $url ) {
		$response = wp_remote_get(
			...
			array(
				'timeout'    => 20,
				'user-agent' => 'SeoSprinkler/' . SPR_VERSION . '; ' . home_url( '/' ),
				'sslverify'  => apply_filters( 'spr_schema_sslverify', true ),
				// Identify ourselves so caching layers can be bypassed if needed.
				'headers'    => array( 'X-SPR-Schema-Check' => '1' ),
			)
		);
		...
		if ( $code < 200 || $code >= 300 ) {
			return array(
				'checked' => false,
				'types'   => array(),
				'error'   => sprintf( /* translators: %d: HTTP status. */ __( 'HTTP %d', 'seo-sprinkler' ), $code ),
			);
		}
	...
	public function required_types() {
		$raw = (string) SPR_Settings::get( 'schema_required_types', '' );
		if ( '' === trim( $raw ) ) {
			return array();
		}
	...
	public function render_publish_box_status( $post ) {
		...
		if ( ! SPR_Settings::get( 'enable_schema_check' ) ) {
			return;
		}
		...
		if ( null === $types ) {
			printf(
				'<div class="misc-pub-section spr-pub-schema"><span class="dashicons dashicons-editor-help"></span> %s</div>',
				esc_html__( 'Schema: not yet checked', 'seo-sprinkler' )
			);
			return;
		}
		...
		printf(
			'<div class="misc-pub-section spr-pub-schema"><span class="dashicons %s"></span> %s <strong>%s</strong></div>',
			esc_attr( $ok ? 'dashicons-yes-alt' : 'dashicons-warning' ),
			esc_html__( 'Schema:', 'seo-sprinkler' ),
			esc_html( empty( $types ) ? __( 'none found', 'seo-sprinkler' ) : implode( ', ', $types ) )
		);
	}
}
