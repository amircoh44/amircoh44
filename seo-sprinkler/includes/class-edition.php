 ...
 * by either defining the SPR_EDITION constant, filtering `spr_edition`, or
 * filtering `spr_validate_license` to turn an entered key into an edition.
 *
...
class SPR_Edition {
	 ...
	 *
	 * Priority: SPR_EDITION constant > stored option > default free, then the
	 * `spr_edition` filter (so a licensing SDK can override).
	 *
	 * @return string
	 */
	public static function current() {
		...
		if ( defined( 'SPR_EDITION' ) && isset( self::$rank[ SPR_EDITION ] ) ) {
			$edition = SPR_EDITION;
		} else {
		...
		$edition = apply_filters( 'spr_edition', $edition );
	...
	public static function free_limit() {
		return (int) apply_filters( 'spr_free_page_limit', self::FREE_LIMIT );
	}
	...
	public static function label( $edition = null ) {
		...
		$labels  = array(
			self::FREE   => __( 'Free', 'seo-sprinkler' ),
	...
	public static function upgrade_url() {
		return apply_filters( 'spr_upgrade_url', 'https://github.com/amircoh44/amircoh44' );
	}
	 ...
	 * provider by filtering `spr_validate_license`.
	 *
	...
	public static function activate_key( $key ) {
		$key     = trim( (string) $key );
		$edition = apply_filters( 'spr_validate_license', self::FREE, $key );
