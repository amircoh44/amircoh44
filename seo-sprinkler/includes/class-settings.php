 ...
 *
 * All plugin options live in a single serialized array under SPR_OPTION_KEY.
 * This class is the only place that knows the shape of that array, so the rest
 * of the plugin can simply call SPR_Settings::get( 'min_images' ).
 *
...
class SPR_Settings {
	 ...
	 */
	const GROUP = 'spr_settings_group';
	...
	public static function install_defaults() {
		$existing = get_option( SPR_OPTION_KEY, array() );
		if ( ! is_array( $existing ) ) {
			$existing = array();
		}
		update_option( SPR_OPTION_KEY, wp_parse_args( $existing, self::defaults() ) );
	}
	...
	public static function all() {
		if ( null === self::$cache ) {
			$stored      = get_option( SPR_OPTION_KEY, array() );
	...
	public static function sanitize( $input ) {
		...
		if ( class_exists( 'SPR_Edition' ) ) {
			SPR_Edition::activate_key( isset( $clean['license_key'] ) ? $clean['license_key'] : '' );
		}
	...
	public static function get_sitemap_urls() {
		 ...
		 */
		return apply_filters( 'spr_sitemap_urls', $urls );
	}
}
