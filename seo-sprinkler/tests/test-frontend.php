...
check( 'internal link injected on phrase', false !== strpos( $rendered, 'spr-internal-link' ) && false !== strpos( $rendered, '>Plumbing Services Experts</a>' ) );
check( 'link points to the target post', false !== strpos( $rendered, get_permalink( $target ) ) );
check( 'distribution block injected', false !== strpos( $rendered, 'CALL-NOW-7788' ) && false !== strpos( $rendered, 'spr-injection' ) );
check( 'CTA placed after first paragraph', strpos( $rendered, 'Second paragraph' ) > strpos( $rendered, 'CALL-NOW-7788' ) );

// Auto-clean on save.
$settings = (array) get_option( 'spr_settings' );
$settings['cleaner_autosave'] = 1;
update_option( 'spr_settings', $settings );
